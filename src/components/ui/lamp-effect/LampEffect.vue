...
const props = defineProps({
  delay: { type: Number, required: false, default: 0.5 },
...
<template>
  ...
  backgroundImage: `conic-gradient(var(--conic-position), var(--tw-gradient-stops))`,
        ...
        <div class="absolute bottom-0 left-0 z-20 h-40 w-full bg-slate-950 mask-[linear-gradient(to_top,white,transparent)]" />
        <div class="absolute bottom-0 left-0 z-20 h-full w-40 bg-slate-950 mask-[linear-gradient(to_right,white,transparent)]" />
      </div>
  ...
  backgroundImage: `conic-gradient(var(--conic-position), var(--tw-gradient-stops))`,
        ...
        <div class="absolute right-0 bottom-0 z-20 h-full w-40 bg-slate-950 mask-[linear-gradient(to_left,white,transparent)]" />
        <div class="absolute right-0 bottom-0 z-20 h-40 w-full bg-slate-950 mask-[linear-gradient(to_top,white,transparent)]" />
      </div>
      ...
      <div class="absolute inset-auto z-50 h-36 w-md -translate-y-1/2 rounded-full bg-cyan-500 opacity-50 blur-3xl" />

      <!-- Spotlight -->
      <div class="animate-spotlight absolute inset-auto z-30 h-36 w-32 -translate-y-24 rounded-full bg-cyan-400 blur-2xl" />

      <!-- Glowing Line -->
      <div class="animate-glowing-line absolute inset-auto z-50 h-0.5 w-60 -translate-y-28 bg-cyan-400" />
