...
watch(() => props.status, (newStatus) => {
  ...
  function animate(now) {
    ...
    currentColors.value = start.map((s, i) =>
      rgbToHex(lerp(s.r, end[i].r, eased), lerp(s.g, end[i].g, eased), lerp(s.b, end[i].b, eased))
    );
...
const styles = computed(() => {
  ...
  return {
    ...
    "--blue-300": c[2], "--blue-400": c[1], "--blue-500": c[0],
    "--indigo-300": c[3], "--violet-200": c[3],
...
<template>
  <div
    v-bind="props"
    :class="cn('transition-bg relative flex h-screen flex-col items-center justify-center bg-zinc-50 text-slate-950 dark:bg-zinc-900', props.class)"
  >
